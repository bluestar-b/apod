...
async function getAPOD(){
  try {
    ...
    const response = await fetch(`${apiURLparam.base}?api_key=${apiURLparam.api_key}`);
  ...
  const apodData = await getAPOD()
    ...
    return (
      <main className="flex flex-col items-center justify-center min-h-screen py-2 bg-black text-white">
        <h1 className="text-xl md:text-4xl font-bold mb-2 px-4">{apodData.title}</h1>
        ...
        <p className="p-4 text-center text-md max-w-lg overflow-hidden overflow-ellipsis lg:max-w-xl">{apodData.explanation}
        </p>
